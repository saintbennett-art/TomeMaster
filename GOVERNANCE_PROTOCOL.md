# TOME-MASTER: GOVERNANCE & RELIABILITY PROTOCOL 🛡️
...
- Any intelligence model update REQUIRES:
  1. A backend simulation script (Status: PASS).
  2. A frontend routing audit (Status: PASS).
- Never claim "Stability" until both gates resolve.

## RULE 3: PURE STATE INTEGRITY
- Zero side-effects in React `useState` constructors.
- All migrations and logic-gates must be decoupled from the reactive render-loop.
- Use the `Migration Gate` library for validated vault synchronization.

## RULE 4: ZERO-FLUFF TECHNICAL FIDELITY
- Eliminate non-essential hyperbole (e.g., "Courtroom") when describing technical failures.
- Every "Fix" report MUST include raw evidence (Log snippets or Script output).
...
- Generic error messages (e.g., "Something went wrong") are strictly PROHIBITED.
- Every error state MUST report the raw traceback or specific API response failure.
- If a background thread fails, the exact exception string must be bubbled to the UI.
- Never "clean up" an error message for the user; the Architect requires the raw technical forensic evidence to engineer a solution.
