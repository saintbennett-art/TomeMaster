...
- **Keys/settings/global prefs**: encrypted vault `settings.enc` (Windows DPAPI, bound to the current user account) via `src/tomemaster/vault_loader.py`; hydrated into env vars at startup. DPAPI is called through `ctypes`/`crypt32.dll` (no pywin32 dependency); `save_vault` fails closed if DPAPI is unavailable. Pre-DPAPI Fernet/XOR vaults are read once and auto-migrated to DPAPI on load. `GET /api/v1/settings/` returns keys **masked**; `/api/v1/analysis/vault-sync` returns presence booleans only. Theme/onboarding/dictionary/UI-layout/last-project live in the vault `preferences` block.
